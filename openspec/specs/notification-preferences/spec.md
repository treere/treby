...
Per-tenant configurability for notification channels. Each type has independent Email and In-app toggles stored as {email, inbox} objects with backward compat for legacy booleans, plus a tenant-level retention setting for read rows.
