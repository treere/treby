...
The system SHALL provide time-series breakdowns of views per job for trend analysis as Contex SVG charts.

#### Scenario: Daily breakdown last 30 days as line chart
- **WHEN** a recruiter views daily frequency for a job with views
- **THEN** the system renders a Contex `PointPlot`/`LinePlot` with `TimeScale` covering the selected period (7/30/90 days), ordered ascending by date, with axis labels and value labels; days with `0` views are included as `0` points on the line

#### Scenario: Daily empty occupies space
- **WHEN** a recruiter views daily frequency for a job where all daily counts are `0` for the selected period
- **THEN** the Daily Views card keeps `min-h-[320px]` and shows a centered dashed placeholder ("No views in this period") instead of rendering an empty chart or collapsing

#### Scenario: Monthly breakdown last 12 months as bar chart
- **WHEN** a recruiter views monthly frequency for a job with views
- **THEN** the system renders a Contex `BarChart` vertical with 12 entries `{month, count}` for each month in last 12 months (months with `0` views as `0` height bars), ordered ascending, with `data_labels: true`

#### Scenario: Monthly empty occupies space
- **WHEN** monthly breakdown has all counts `0`
- **THEN** the Monthly card keeps `min-h-[320px]` and shows a centered placeholder ("No monthly data yet")
...
The system SHALL break down views by traffic source when available as a Contex PieChart.

#### Scenario: Source breakdown by utm_source and referer as pie
- **WHEN** a recruiter views source breakdown for a job with views
- **THEN** the system renders a Contex `PieChart` grouped by `utm_source` if present, otherwise by `referer` domain, otherwise as `"Direct"`, with `legend_setting: :legend_right` and counts/percentages visible; data is `[{source, count, percentage}]` ordered descending by count

#### Scenario: Source empty occupies space
- **WHEN** source breakdown is empty (no views)
- **THEN** the Traffic Sources card keeps `min-h-[320px]` and shows a centered placeholder ("No source data yet") instead of collapsing

#### Scenario: No source data
- **WHEN** all views for a job have no `utm_source` and no `referer`
- **THEN** source breakdown returns a single entry `{source: "Direct", count: total_views, percentage: 100}` and renders a pie with one slice

### Requirement: Per-job analytics page
The system SHALL provide a dedicated analytics page for each job, accessible from the job management area, where all visualizations are Contex SVG charts with stable card heights.
...
#### Scenario: Analytics page content as charts
- **WHEN** a team member navigates to `/app/jobs/:id/analytics`
- **THEN** they see: job title header with back link to job detail, KPI cards (total views, unique views, views last 7/30 days, avg daily, conversion rate), a daily Contex line chart (with 7/30/90 selector that re-renders server-side), a monthly Contex bar chart for last 12 months, a sources Contex pie chart with legend, and the view→application funnel with tenant average

#### Scenario: Analytics page for job with no views
- **WHEN** a team member views analytics for a job with zero views
- **THEN** KPI cards show `0`/`N/A`, the summary empty state is shown, and each chart card keeps its height with a centered placeholder and explanatory text (no collapsed cards)
...
- **THEN** its analytics page remains accessible and shows historical Contex charts (no new views are tracked after closing)
