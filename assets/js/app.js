...
document.addEventListener(
  ...
  (e) => {
    ...
    if (form.hasAttribute("phx-submit") || form.hasAttribute("data-phx-submit"))
      return;
    ...
    buttons.forEach((btn) => {
      ...
      const loadingText =
        btn.getAttribute("phx-disable-with") ||
        btn.getAttribute("data-loading-label");
      if (loadingText) {
...
document.addEventListener("click", (e) => {
  ...
  wrapper
    ?.querySelectorAll(".password-show-icon")
    .forEach((el) => el.classList.toggle("hidden", !showing));
  wrapper
    ?.querySelectorAll(".password-hide-icon")
    .forEach((el) => el.classList.toggle("hidden", showing));
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (window.liveSocket) {
          el.dispatchEvent(
            new CustomEvent("phx:clear-flash", {
              bubbles: true,
              detail: { key: kind },
            }),
          );
...
new MutationObserver(initFlashAutoDismiss).observe(document.body, {
  childList: true,
  subtree: true,
});
