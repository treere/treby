...
- **THEN** the system generates a 6-digit numeric code, stores its SHA-256 hash in `candidate_otps` with a 10-minute expiry, and sends an email containing the code to the candidate's address and the UI shows "Code valid 10 minutes — check spam folder, sender noreply@treby.app" on both login and verify screens

#### Scenario: Email does not exist
- **WHEN** a candidate enters an email that does not match any candidate record in the tenant
- **THEN** the system displays the same success message as for existing emails ("Check your email for your login code") to prevent email enumeration and the same static helper text (expiry/spam hint) without revealing non-existence

#### Scenario: Repeated requests rate limited
- **WHEN** a candidate requests a code more than once within 60 seconds
- **THEN** the system does not send a new code, keeps the generic success message, and the UI shows a cooldown hint "Wait 60s before requesting another code" with a live countdown on the verify screen
...
The system SHALL verify the code entered by the candidate against the stored OTP and create an authenticated session on success, with clear expiry and typo-recovery guidance.
...
- **THEN** the system increments the attempt counter and displays a generic "invalid or expired code" error with a link "Didn't receive it? Check spam or correct your email" back to `/portal/login`
