...
Ensure that supported locales (IT/EN) have complete translations and that the POT catalog stays fresh, by providing automated guards that run locally and in CI and block merges when coverage is incomplete. A complementary Credo check flags hardcoded UI strings before extraction.
