defmodule Treby.Calendar.Google do
  @moduledoc """
  Google Calendar API client using Req.
  Handles FreeBusy queries, event creation with Google Meet, and event deletion.
  """
  ...
  def free_busy(%Treby.Calendar.CalendarConnection{} = conn, time_min, time_max) do
  ...
  def delete_event(%Treby.Calendar.CalendarConnection{} = conn, event_id) do
    with {:ok, token} <- get_valid_token(conn) do
      req =
        Req.new(base_url: @base_url)
        |> Req.Request.put_header("authorization", "Bearer #{token}")
