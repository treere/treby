...
- **Server-side form application**: when the assistant proposes corrected or pre-filled form values for the form currently in context and the user confirms, the values are applied directly to the live form on the host page via the host LiveView (not advisory-only). Application runs server-side and is scoped to the form in context.
...
- **Out of scope**: multi-agent/Jido; conversation history browser; bulk confirm-all; retention pruning.
