...
- **THEN** the system displays a list of all their applications across all jobs, each showing: job title, current pipeline stage (with colored badge plus human-readable label, e.g., "Received" for `new`), last activity timestamp, and a preview of the most recent message (if any)
...
The system SHALL render the candidate portal with a distinct, simplified layout that is usable on mobile.

#### Scenario: Portal layout
- **WHEN** candidate accesses any portal route
- **THEN** the system renders a layout with: tenant branding (logo, primary color), a simple navigation (Dashboard, Settings, Messages, Schedule), and the candidate name in the header
