...
**Reason**: The public booking token mechanism is removed; self-scheduling happens inside the authenticated candidate portal.
**Migration**: Candidates access `/:tenant_slug/portal/schedule` after logging in via OTP; no `booking_tokens` are created.

### Requirement: Public booking page
**Reason**: The public booking page `/:tenant_slug/schedule/:token` is replaced by an authenticated booking page in the candidate portal.
**Migration**: Candidates book from `/:tenant_slug/portal/schedule` after logging in via OTP.

### Requirement: Email booking link to candidate
**Reason**: The booking link email is removed; the candidate is notified via the portal conversation and an optional ping email.
**Migration**: See `candidate-booking-email` removal and the interview notification flow in `interview-scheduling`.
